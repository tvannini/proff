...
if (!defined('ASPELL_ULTRA')) {
    define(ASPELL_ULTRA, 4);        # Ultra fast mode (not available in Pspell!)
    }
...
class Aspell{
     ...
     * @author   Andreas Gohr <andi@splitbrain.org>
     */
    function Aspell($aspellbin, $language, $jargon = null, $encoding = 'CP1252') {

        $this->aspellbin = $aspellbin;
    ...
    function setMode($mode) {
        ...
        if (!in_array($mode, array(PSPELL_FAST,
                                   PSPELL_NORMAL,
                                   PSPELL_BAD_SPELLER,
                                   ASPELL_ULTRA))) {
            $mode = PSPELL_NORMAL;
    ...
    function runAspell($text, &$out, &$err, $specials = null) {
        ...
        if (strpos(strtolower(PHP_OS), "win") !== false) {
            $prefix = " --prefix=\"".dirname(dirname($this->aspellbin))."\"";
            }
        else {
        	$prefix = "";
            }
        $command = $this->aspellbin.' -a'.$this->args.$prefix."\n";
        $stdin   = '';
        // prepare specials
        ...
        if (is_null($rc)) {
            $err = "Could not run Aspell '".$this->aspellbin."'";
            return false;
            }
        ...
        if ($err){
            ...
            $err = "Aspell returned an error(".$this->aspellbin." exitcode: $rc ):\n".
                   $err;
            return false;
            }
        return true;
     ...
     * exit code is returned. If the pipe couldn't be opened null is returned
     *
